...
router.patch("/profile/edit", userAuth, async (req, res) => {
  const { _id } = req.user;
  const allowedUpdateFields = ["gender", "age", "about", "skills", "photoUrl"];
  const skills = req.body.skills.split(",").map((skill) => skill.trim());
  const { age, about, gender, photoUrl } = req.body;
  try {
    ...
    if (user === null) {
      ...
      const updatedProfile = await User.findByIdAndUpdate(
        { _id: _id },
        { age, about, gender, photoUrl, skills },
        {
...
router.patch("/profile/edit/password", userAuth, async (req, res) => {
  ...
  try {
    ...
    if (!validateCreds) {
      // clearing the cookie in case client dont know the old password
