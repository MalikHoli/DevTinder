...
const { viewProfile } = require("../utils/viewProfile");
...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const IsSignupAllowed = Object.keys(req.body).every((k) =>
      allowedSignupFields.includes(k)
    );
    if (!IsSignupAllowed) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (validateCreds) {
      ...
      res.send(viewProfile(user._doc));
    } else {
