...
router.get("/user/requests/received", userAuth, async (req, res) => {
  //To get all the connection requests received and pending to respond
  try {
    const loggedInUser = req.user._id;
    const status = "interested";
    //*******************************************************************************************************************
    //Finding connections that fullfill the criteria to fetch all the connections to which logged in user sent request
    const connectionRrequestReceived = await ConnectionRequests.find({
...
router.get("/user/requests/match", userAuth, async (req, res) => {
  //To get matched connections (kind of like FB friends)
  try {
    const loggedInUser = req.user._id;
    ...
    const connectionRrequestReceived = await ConnectionRequests.find({
      ...
      .populate(
        "toUserId",
        "firstName lastName age gender about skills photoUrl"
      ); //We can chain the populate like this

    const match = connectionRrequestReceived.map((ele) => {
...
router.get("/feed", userAuth, async (req, res) => {
  try {
    ...
    const interactionConnections = await ConnectionRequests.find({
    ...
    const feedUsers = await User.find({
      $nor: [{ _id: [...interactedUsersIds] }, { _id: loggedInUser }],
    })
